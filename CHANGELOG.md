...
- **New `input` namespace (13 actions) for Enhanced Input.** `UInputAction` and `UInputMappingContext` inspection and authoring. It lives in `MonolithGAS` for proximity to ability input binding but it is **not GAS** — its assets are engine assets, and it registers *before* the `bEnableGAS` gate, so Input Actions and Mapping Contexts stay reachable when GAS integration is switched off entirely.
  - **Read-only inspection (5):** `list_input_actions`, `get_input_action`, `list_input_mapping_contexts`, `get_input_mapping_context`, `validate_input_mappings`. Canonical mounted package or matching top-level object paths only. Every page, instanced array and validation scan carries a documented hard bound, and validation separates the mapping-scan's `page_complete` from pagination-level `all_contexts_covered` — `complete` needs both, and `valid` additionally needs zero errors. Duplicate key assignments are warnings, not errors. Missing actions, invalid keys, load failures and truncated scans fail explicitly; no fallback asset is ever substituted. None of the five transact, save, mutate or dirty a package. Reimplemented from PR #130 by **@kunkunGames**.
  - **Authoring (8):** `add_mapping_modifier`, `add_mapping_trigger`, `remove_mapping_modifier`, `remove_mapping_trigger`, `set_mapping_modifiers`, `set_mapping_triggers`, `set_input_action_modifiers`, `set_input_action_triggers`. Mappings are addressed by `mapping_index` or by an `(input_action, key)` pair; **an ambiguous pair is refused rather than guessed**.
- **New read-only `localization` namespace (4 actions).** Registered by `MonolithConfig` **ahead of** the `bEnableConfig` gate, so culture and StringTable inspection stay available when config authoring is off. The dispatcher is annotated read-only and idempotent.
  - `list_cultures` — bounded culture discovery (limit 1-500, default 100), with explicit-root resolution (`culture_names`, max 256, optional `include_derived`) and an `unresolved_names` list for roots Unreal could not resolve.
  - `list_string_tables` — AssetRegistry StringTable discovery under a canonical mounted package root (limit 1-1000, default 200). `include_details` loads **only the returned page** and adds namespace, table id, internal flag and entry count.
  - `get_string_table` — a bounded entry page in stable key order with an exclusive `after_key` cursor (entry limit 1-1000, cursor max 4096 chars), plus independent metadata-row (`metadata_limit` 0-4096, shared across the page) and text (`text_limit` 1-65536) budgets. Readback separates `has_more_entries`, `all_entries_covered` and `metadata_complete`, so a caller can tell *which* budget stopped it.
  - `validate_string_table` — deterministic bounded key/source validation (scan limit 1-10000) with paginated issues. Reports `valid=true` **only** when the scan covered every entry and found zero errors; a truncated scan is itself an error.
  - All four require canonical mounted package paths (or the matching top-level object path) and never transact, save, mutate or dirty a package. StringTable *authoring* remains on the `blueprint` namespace.
- **6 bounded read-only `chooser` actions** — `list_chooser_tables` (AssetRegistry discovery with boundary-exact package-prefix filtering, so `/Game/Choosers` never matches `/Game/ChoosersOld`), `get_chooser_table`, `list_chooser_columns`, `list_chooser_rows`, `list_chooser_references`, `validate_chooser_table`. Every response is finite: independent table / row / column / reference / depth / field / container / string / global-visit bounds, each cutoff reported through explicit truncation or `complete` metadata — **a short answer is never indistinguishable from a whole one**. Column readback reads the ACTIVE per-row array (`RowValuesWithAny`) rather than the UHT-registered deprecated `RowValues`, so bool columns no longer report zero cells, and stale `CookedResults` never inflate the authoritative editor row count. Canonical paths only: aliases, redirectors, backslash spellings and case-only variants are rejected rather than silently normalized. All six stay registered and discoverable when the optional Chooser plugin is disabled, returning an explicit availability error instead of synthesized data. From PR #129 by **@kunkunGames**.
  - **`validate_chooser_table` is DISTINCT from the existing `validate_chooser`.** The new one never compiles, mutates, saves or dirties a package; the old one still performs its compile-oriented pass. Neither replaces the other.
- **User Defined Structs are editable after creation** — `get_struct_fields`, `add_struct_field`, `remove_struct_field`, `rename_struct_field`, `set_struct_field_type`. `create_user_defined_struct` authored a struct once; there was no way to change one afterwards and no way to read one at all. No new engine surface was needed — `FStructureEditorUtils` already drove `create_user_defined_struct` and was simply unreachable for an existing asset. Fields are targeted by display name (`Mobility`) rather than the serialized `VarName` (`Mobility_36_31089BED...`), though either resolves, and a miss lists the names that do exist. `get_struct_fields` reports types in the grammar the writers parse, containers included, so its output feeds straight back into `add_struct_field`. Each writer reports what it recompiled, what it dirtied, and which assets reference the struct (they are **not** recompiled here); `set_struct_field_type` additionally reports the default value the engine discards, because a retype is a migration, not a rename. Three engine behaviours are answered rather than passed through: `RemoveVariable` returns false for both "would empty the struct" and "no such field", `ChangeVariableType` / `RenameVariable` return false for a no-op, and `MoveVariable` returns false when the field is already in the requested slot.
- **`ui build_ui_from_spec` gained a `mode`: `"rebuild"` (default, unchanged) or `"patch"`.** Patch matches spec nodes to existing widgets by id + exact class, reuses those widget and slot objects, reorders them to spec order, and adds or removes only what the spec adds or removes — so custom `WidgetStyle` tints, `BackgroundBlur.BlurStrength`, tooltips and unmodelled slot fields survive. An unchanged `FText` is left untouched, preserving its localization namespace and key. `ui build_menu_from_spec` takes the same `mode` and reports `aggregate_data_loss`; `ui dump_ui_spec_schema` documents both modes under `build_modes`. An unrecognised `mode` is rejected with `-32602` — it never falls back to rebuild.
- **`editor start_pie` gained an `on_compile_errors` policy.** Loaded Blueprints are pre-flighted against the engine's own unresolved-error condition (`BS_Error && bDisplayCompilePIEWarning`, the exact test `FInternalPlayLevelUtils::ResolveDirtyBlueprints` uses). `"refuse"` (default) returns the offending `{name, path}` list without starting PIE; `"suppress"` starts anyway under a scoped `GIsRunningUnattendedScript` guard so `ShowBlueprintErrorDialog` early-outs instead of running a nested modal loop that starves the in-process MCP server. Unrecognised policies are rejected as `-32602` before any editor-state check. Successful starts report `compile_error_policy`, `errored_blueprint_count` and `errored_blueprints`. Thanks **@Hvizeu**.
- **`editor capture_pie_movement_clip` gained `include_ui`** (default `false`). With it set, frames are captured through the engine screenshot path (`FScreenshotRequest::RequestScreenshot`, the `shot showui` mechanism) so Slate composites UMG into the backbuffer before the PNG is written — the default `FViewport::ReadPixels` path only ever sees the scene render target, which is why UI iteration used to come back as an empty scene. These frames are written asynchronously at end of frame, so per-frame uniformity is not verified for them; `capture_validity` reports `uniformity_checked: false` plus a note. With PIE docked in the editor the backbuffer is the whole editor window — use new-window PIE for a clean game+UI clip. Thanks **@whalemenace**.
- **`editor create_empty_map` gained `open`** (default `false`) **and `dirty_policy`.** `open=true` loads the newly created map by delegating to `editor load_level`, inheriting its dirty-current-map refusal, PIE-teardown guard and stale-resident-world guard. Thanks **@whalemenace**.
- **`editor capture_scene_preview` now honours scalar `width` / `height` params** (default 512 each, so existing callers are unchanged) alongside the existing `resolution: [w,h]` array. Scalars override the matching array element. Both are declared in the action schema, so they no longer come back as `Unknown param` warnings while the capture silently stayed 512x512. Reported by **@Alexbeav** (#141).
- **`project search` gained an `asset_class` filter.** Takes a bare string (`"Blueprint"`) or an array (`["Blueprint","WidgetBlueprint"]`), matches case-insensitively, de-duplicates, and is capped at 32 entries; absent means unfiltered. Searching a common token in a project with a large third-party content folder previously buried the target — a real `E_` search returned 16 `UserDefinedEnum` rows under 21 `Texture2D`, 10 `NiagaraEmitter` and 3 `StaticMesh`. **The predicate is applied inside the SQL** (`AND a.asset_class COLLATE NOCASE IN (?,?,...)`), not as a pass over the returned array, so `limit` counts *matching* rows rather than returning the top N of everything followed by a cull. Both FTS statements already `JOIN assets`, so a graph-node text hit inside a Blueprint still survives a `Blueprint` filter. Only the placeholder count is interpolated; every class name stays a bound parameter. `asset_class` is type-checked against `EJson` rather than read through `TryGetStringField`, which coerces — a numeric `7` would otherwise become a filter for a class of that name (zero results dressed up as a valid search) instead of a `-32602`. Whitespace-only input and over-cap lists are likewise `-32602`; a class no asset uses is a successful empty result. Responses echo the applied filter as `asset_class_filter`. Mirrored into `monolith_query.exe` and `monolith_offline.py` (both accept `--asset-class A,B`; the Python tool also accepts a repeated flag), with two new automation tests — `Monolith.ProjectSearch.AssetClassFilter` uses a lopsided 10-noise/3-wanted fixture that a post-hoc filter cannot pass by luck.

### Fixed

- **Instanced Enhanced Input modifiers and triggers could not be authored from script (#140, reported by @Alexbeav).** Modifiers nested in an Input Mapping Context's mapping array were silently dropped on save: the mapping slots kept their count, but every instanced subobject read back as `None` after a reload, with nothing in the editor or the log flagging the loss. The new actions construct each modifier and trigger with the outer and flags the details panel itself uses, so the subobject is harvested into the mapping context's package instead of being written as an unresolvable import. Unknown property names on an authored modifier are now rejected instead of quietly ignored.
- **`blueprint add_node` no longer piles every unplaced node on the origin (#132, reported by @Nicopara, diagnosed by @Alexbeav).** Omitting `position` defaulted to `[0,0]`, so a graph authored through MCP came out as one unreadable stack. Nodes with no explicit position are now placed in clear space relative to the graph's existing content; an explicit `position` is still authoritative, the first node in an empty graph still lands at `[0,0]`, and the response reports the chosen position plus `position_source`.
- **`blueprint auto_layout(layout_mode: "new_only")` now moves piles, not just nodes at the origin.** Its rule was "unpinned only if at (0,0)" — the second half of the comment above it (`and NOT heavily overlapping`) had never been written, which made the mode depend on the origin-stacking bug above. A node is now treated as new if it is at the origin *or* piled on another node, measured by intersection area against the smaller node (25%), so edge contact in a hand-laid graph is left alone. The response gained `nodes_pinned`, so "moved nothing" is no longer indistinguishable from "nothing needed moving". Note the consequence: because `add_node` now lands unpositioned nodes in clear space, those nodes are *not* `new_only` candidates — use `layout_mode: "all"` to fold them into the flow.
- **`ui build_ui_from_spec` rebuild mode audits what it is about to destroy (#139).** Every editable property that differs from its class default and is not restored by the spec builders is reported as a warning naming the widget and dotted path (e.g. `Button_Play.WidgetStyle.Normal.TintColor`), plus one warning per keyed `FText` whose loc key the rebuild would reassign. The audit runs on `dry_run: true` too, so the loss can be inspected before it happens. The build response now carries `mode`, `teardown`, top-level `error_count` / `warning_count`, `node_counts.reused`, a `node_counts.semantics` string spelling out the teardown, and a `data_loss { property_count, widget_count, localization_keys_reset, properties[] }` block. `warnings[]` now includes validator-surface findings, so its length matches `warning_count`.
- **`editor create_empty_map` no longer implies it changed the open world.** It never did — it saves a `UWorld` asset and leaves the editor on whatever level was loaded, so agents chaining create -> populate silently edited the wrong world. The response now always carries `current_world` (the editor world after the call, as a package name directly comparable to `path`) and `opened`, and the success message states outright that the open world is unchanged and how to switch to the new map.
- **`niagara get_module_input_value` / `get_module_inputs` no longer report `(default)` for module inputs set in the Niagara stack editor (#143).** Both now read the owning script's rapid-iteration parameter store when an input has no graph override pin, and report `source: "rapid_iteration"`. Values are formatted with the engine's own pin-literal writer, so they round-trip through `set_module_input_value`. Only values that differ from the module's declared default are reported as set, since the engine seeds the rapid-iteration store with defaults at compile time.
- **`niagara set_module_input_value` now writes LinearColor correctly (#122, #143).** The JSON object form (`{"r":..,"g":..,"b":..,"a":..}`) emitted a comma-separated literal, which the engine's colour pin parser rejects and silently replaces with black-with-alpha-1 — the "only alpha applied" symptom. Colour now emits `(R=..,G=..,B=..,A=..)` and vec2 emits `X=.. Y=..`, matching the engine's per-type pin syntax. Object values that arrive as a serialized JSON string are also accepted.
- **`audio add_sound_cue_node` and four sibling Sound Cue actions no longer crash the editor when the target cue's asset editor is closed (#125, reported by @M0rtaI).** `FinalizeCue` called `USoundCue::LinkGraphNodesFromSoundNodes()` unconditionally; the engine implementation `CastChecked`s every sound node's editor graph-node back pointer, which is a fatal assert (`Cast of nullptr to SoundCueGraphNode failed`) for a cue whose editor graph was never created. Sound Cue edits now rebuild a missing editor graph before touching it (`USoundCue::CreateGraph` / `SetupSoundNode`) and skip the graph link entirely when it still cannot be linked safely, instead of asserting. The runtime node chain is always updated either way. This also covers `audio connect_sound_cue_nodes`, which crashed earlier still: `USoundNode::InsertChildNode` routes through `FSoundCueAudioEditor::CreateInputPin` and `CastChecked`s the same back pointer before `FinalizeCue` is ever reached. All twelve Sound Cue mutation actions now report `graph_link_skipped: true` in their JSON response (and log a warning) when the visual graph link had to be skipped.
- **Quitting the editor while a project index was running hung the process forever.** The indexing worker parked in an unbounded `FEvent::Wait()` at fourteen points, every one waiting on work only the game thread can run, while `Deinitialize()` blocked the game thread joining that same worker. Waits now poll in 100ms slices and abandon only when the game thread has provably stopped serving them (shutdown join or engine exit), so a force-stopped index exits in well under a second. A user-initiated cancel is unaffected and still runs to a clean finish.
- **An abandoned game-thread dispatch is now withdrawn through an abort token before the worker unwinds**, so its payload can never run against a retired stack frame, and the asset-registry scan owns its state outright instead of borrowing that frame.
- **Interrupting an index by closing the editor no longer trips `"AsyncTaskNotification was still pending when destroyed"`:** `Deinitialize` completes the progress notification before destroying it. The deadlock had been hiding this assert.
- **Cleared the UE 5.8 C4996 deprecation warnings (#121, reported by @aggitti) while keeping UE 5.7 compilation intact.**
  - `GetObjectsWithOuter` / `ForEachObjectWithOuter`: dropped the deprecated `bIncludeNestedObjects` boolean. Sites that passed `true` now rely on the default (identical on both engines); the one site that passed `false` is version-gated to `EGetObjectsFlags::None` on 5.8.
  - `FCoreDelegates::OnPostEngineInit`: routed the MonolithAI / MonolithLevelSequence / MonolithUI listeners through a new `MonolithCoreDelegatesCompat::GetOnPostEngineInit()` shim that resolves to the 5.8 accessor or the 5.7 data member.
  - Animation: `UMirrorDataTable::FindReplaceMirroredNames` -> `UpdateFromFindReplaceExpressions(FFindReplaceOptions::AddMissingOnly())` (5.8 only, gated); `FIKRetargetOpBase::SetSettings` -> `GetSettings()->CopySettingsAtRuntime()` (ungated); PoseSearch database entry edits now read a copy and commit it via `SetAnimationAssetAt` on 5.8 instead of mutating through the deprecated mutable accessor.
  - `REN_ForceNoResetLoaders` removed from all `Rename` calls — the flag is already inert on UE 5.7 and deprecated on 5.8. `REN_AllowPackageLinkerMismatch` is deliberately NOT used in its place.

### Changed

- **`editor capture_scene_preview` validates capture size.** Non-positive width/height and sides beyond 8192 px are rejected as `-32602` invalid params; the widget branch's scale-adjusted physical target is clamped to the same ceiling.
